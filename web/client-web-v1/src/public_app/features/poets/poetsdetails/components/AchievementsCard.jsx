...
const AchievementsCard = ({ achievements = [] }) => {
  ...
  // Process achievements for multilingual support
  const processedAchievements = achievements.map((achievement) => {
    if (typeof achievement === "object") {
      return achievement[language] || achievement.en || "";
    }
    return achievement;
  });
  ...
  if (!achievements || achievements.length === 0) {
    return (
      ...
      >
        <h3
          className={`font-bold text-lg ${textPrimary} mb-4 flex items-center gap-2`}
        >
          <FaAward className={iconColor} />{" "}
          {t("notableAchievements") || "Notable Achievements"}
        </h3>
        <p className={textTertiary}>
          {t("noAchievements") || "No achievements listed"}
        </p>
      </div>
    );
  }
      ...
      <ul className="space-y-3">
        {processedAchievements.map((achievement, idx) => (
          <li key={idx} className="flex items-start gap-2 group/item">
            <FaStar
              className={`${iconColor} text-sm mt-1 flex-shrink-0 transition-transform group-hover/item:scale-110`}
            />
